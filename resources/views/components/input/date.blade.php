<div x-data="{ value: @entangle($attributes->wire('model')) }" x-init="new Pikaday({ field: $refs.input, format: 'YYYY-MM-DD'});" @change="value = $event.target.value" class="max-w-lg flex rounded-md shadow-sm">
 ...
 <input x-ref="input" :value="value" autocomplete="off" {{ $attributes->whereDoesntStartWith('wire:model') }} class="rounded-none rounded-r-md flex-1 form-input block w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5"/>
</div>
@push('css')
